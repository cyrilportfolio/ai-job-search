...
- **Fetching:** an honest User-Agent that names the tool, version, and purpose — `<portal>-cli/1.0 (personal job search)` — with **no** `Mozilla/5.0 (compatible; ...)` prefix. That prefix is the shape well-known crawlers (Googlebot, Bingbot) self-declare with, and a controlled A/B test against a real site's WAF (`legrand-associates.com`, 2026-08-12 — see `.agents/skills/legrand-search/url-reference.md`) found it triggers a 403 that the identical request with the plain form does not: dropping the prefix is *more* honest identification, not evasion, and it's strictly safer. Never a full browser impersonation either way; if the portal refuses even the honest UA, escalation to browser headers goes through the robots.txt gate in `.claude/skills/job-application-assistant/09-web-research.md`, not through the CLI's default. Exponential backoff with jitter on 429/5xx (max ~6 retries), `""`/`null` on 404 rather than a crash.
